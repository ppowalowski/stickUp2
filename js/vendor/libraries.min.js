 ...
 * Released under the MIT license
 */
function(factory){"function"==typeof define&&define.amd?define(["jquery"],factory):factory("object"==typeof exports?require("jquery"):jQuery)}(function($){function encode(s){return config.raw?s:encodeURIComponent(s)}function decode(s){return config.raw?s:decodeURIComponent(s)}function stringifyCookieValue(value){return encode(config.json?JSON.stringify(value):String(value))}function parseCookieValue(s){0===s.indexOf('"')&&(s=s.slice(1,-1).replace(/\\"/g,'"').replace(/\\\\/g,"\\"));try{return s=decodeURIComponent(s.replace(pluses," ")),config.json?JSON.parse(s):s}catch(e){}}function read(s,converter){var value=config.raw?s:parseCookieValue(s);return $.isFunction(converter)?converter(value):value}var pluses=/\+/g,config=$.cookie=function(key,value,options){if(void 0!==value&&!$.isFunction(value)){if(options=$.extend({},config.defaults,options),"number"==typeof options.expires){var days=options.expires,t=options.expires=new Date;t.setTime(+t+864e5*days)}return document.cookie=[encode(key),"=",stringifyCookieValue(value),options.expires?"; expires="+options.expires.toUTCString():"",options.path?"; path="+options.path:"",options.domain?"; domain="+options.domain:"",options.secure?"; secure":""].join("")}for(var result=key?void 0:{},cookies=document.cookie?document.cookie.split("; "):[],i=0,l=cookies.length;l>i;i++){var parts=cookies[i].split("="),name=decode(parts.shift()),cookie=parts.join("=");if(key&&key===name){result=read(cookie,value);break}key||void 0===(cookie=read(cookie))||(result[name]=cookie)}return result};config.defaults={},$.removeCookie=function(key,options){return void 0===$.cookie(key)?!1:($.cookie(key,"",$.extend({},options,{expires:-1})),!$.cookie(key))}}),jQuery(function(){function filterPath(string){return string.replace(/^\//,"").replace(/(index|default).[a-zA-Z]{3,4}$/,"").replace(/\/$/,"")}function scrollableElement(els){for(var i=0,argLength=arguments.length;argLength>i;i++){var el=arguments[i];if($scrollElement=$(el),$scrollElement.scrollTop()>0)return el;$scrollElement.scrollTop(1);var isScrollable=$scrollElement.scrollTop()>0;if($scrollElement.scrollTop(0),isScrollable)return el}return $("body")}var lastClicked,locationPath=filterPath(location.pathname),scrollElem=scrollableElement("html","body"),$scrollElem=$(scrollElem);$(window).on("scroll",function(e){}),$scrollElem.on("click.smoothScroll","a[href*=#]:not([data-noscroll])",function(e){this&&e.preventDefault();var thisPath=filterPath(this.pathname)||locationPath;if(locationPath!=thisPath||location.hostname!=this.hostname&&this.hostname||!this.hash.replace(/#/,""))window.location=jQuery(this).attr("href");else{var $target=$(this.hash),target=this.hash;if($target.length){var targetOffset=$target.offset().top;if(lastClicked==this)return;$scrollElem.stop(),lastClicked=this,$scrollElem.animate({scrollTop:targetOffset},400,function(){location.hash=target,lastClicked=!1})}}})});